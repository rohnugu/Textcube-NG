...
// Ver:		$Id: jpgraph.php,v 1.242.2.75 2004/06/20 09:27:14 aditus Exp $
//
...
class JpGraphErrObjectImg {
	...
	$erricon = Picture::CreateFromString(base64_decode($img_iconerror));   
...
function __construct($aWidth=300,$aHeight=200,$aCachedName="",$aTimeOut=0,$aInline=true) {
	...
	$this->img	= new RotPicture($aWidth,$aHeight);
...
// PUBLIC METHODS	
    ...
    function SetBackgroundPicture($aFileName,$aBgType=BGIMG_FILLPLOT,$aImgFormat="auto") {
	...
	if( $aImgFormat == "auto" ) {
	    $e = explode('.',$aFileName);
	    if( !$e ) {
		JpGraphError::Raise('Incorrect file name for Graph::SetBackgroundPicture() : '.$aFileName.' Must have a valid image extension (jpg,gif,png) when using autodetection of image type');
	    }
		...
		JpGraphError::Raise('Unknown file extension ($aImgFormat) in Graph::SetBackgroundPicture() for filename: '.$aFileName);
	    }    
	}
    ...
    function SetBackgroundPictureMix($aMix) {
	$this->background_image_mix = $aMix ;
    }
	
    // Adjust brightness and constrast for background image
    function AdjBackgroundPicture($aBright,$aContr=0,$aSat=0) {
    ...
    function AdjPicture($aBright,$aContr=0,$aSat=0) {
    ...
    function GetHTMLPictureMap($aMapName) {
	...
	if( empty($_REQUEST[_CSIM_DISPLAY]) ) {
		...
		$htmlwrap = $this->GetHTMLPictureMap($aCSIMName)."\n".
		...
		echo $this->GetHTMLPictureMap($aCSIMName);
	...
	if( !$_csim ) {
	    ...
	    if(_JPG_DEBUG) {
		$this->DisplayClientSideaPictureMapAreas();		
	    }
...
// PRIVATE METHODS	
    ...
    function LoadBkgPicture($aImgFormat='',$aFile='') {
	if( $aFile == '' )
	...
	if( $this->background_image != "" ) {
	    $bkgimg = $this->LoadBkgPicture($this->background_image_format);
...
//	$this->img->FilledRectangle(0,0,$this->img->width-1-$hadj,$this->img->height-1-$vadj); 
    ...
    function DisplayClientSideaPictureMapAreas() {
...
// PUBLIC METHODS	
	...
	// Add us as an observer to class Picture
...
function __construct($aWidth,$aHeight,$aFormat=DEFAULT_GFORMAT) {
	...
	*/
	...
	$bbox = @PictureTTFBBox($this->font_size,$aAngle,$this->font_file,$aTxt);
	if( $bbox === false ) {
	...
	if( !is_readable($this->font_file) ) {
	    JpGraphError::Raise('Can not read font file ('.$this->font_file.') in call to Picture::GetBBoxTTF. Please make sure that you have set a font before calling this method and that the font is installed in the TTF directory.');
	}
	...
	if( strpos($txt, "\n") === false || ($dir>0 && strpos($txt, "\n") !== false) ) {
	    ...
	    PictureTTFText ($this->img, $this->font_size, $dir, $x, $y, 
			  $this->current_color,$this->font_file,$txt); 

            if( $debug ) {
		// Draw the bounding rectangle and the bounding box
		$box=@PictureTTFBBox($this->font_size,$dir,$this->font_file,$txt);
		$p = array();
		$p1 = array();
		for($i=0; $i < 4; ++$i) {
		    $p[] = $bbox[$i*2]+$x;
		...
		$yl = $y  - $yadj; 
		$xl = $xl  - $xadj; 
		PictureTTFText ($this->img, $this->font_size, $dir, 
			      $xl, $yl-($h-$fh)+$fh*$i,
		...
		if( $debug  ) {
		    ...
		    $box=@PictureTTFBBox($this->font_size,$dir,$this->font_file,$tmp[$i]);
		    $p = array();
		    for($j=0; $j < 4; ++$j) {
			$p[] = $bbox[$j*2]+$xl;
	...
	if( $this->current_color == -1 ) {
	    ...
	    JpGraphError::Raise("Can't allocate any more colors.
				Picture has already allocated maximum of <b>$tc colors</b>. 
...
DEFINE(\"USE_APPROX_COLORS\",true);
	...
	if( $bc == -1 ) {
	    JpGraphError::Raise('Picture::FillToBorder : Can not allocate more colors');
	    exit();
	}
...
//===================================================
// CLASS RotPicture
// Description: Exactly as Picture but draws the image at
// a specified angle around a specified rotation point.
//===================================================
class RotPicture extends Picture {
    var $m=array();
    var $a=0;
...
function __construct($aWidth,$aHeight,$a=0,$aFormat=DEFAULT_GFORMAT) {
    ...
    function FilledCircle($xc,$yc,$r) {
	// If we use GD1 then Picture::FilledCircle will use a 
...
// PUBLIC METHODS	
    ...
    function PutAndStream(&$aPicture,$aCacheFileName,$aInline,$aStrokeFileName) {
	...
	if( $aStrokeFileName!="" ) {
	    ...
	    $aPicture->Stream($aStrokeFileName);
	    return;
	}
	...
	if( $aCacheFileName != "" && USE_CACHE) {
		...
		if( !@unlink($aCacheFileName) )
		    JpGraphError::Raise(" Can't delete cached image $aStrokeFileName. Permission problem?");
		$aPicture->Stream($aCacheFileName);	
	    }
	    else {
